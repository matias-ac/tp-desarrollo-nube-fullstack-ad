...
export function Login() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    // Simulation of authentication success
    navigate('/dashboard');
  };
    ...
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden bg-slate-50">
      ...
      <div className="relative z-10 w-full max-w-md bg-white rounded-2xl shadow-xl p-8 border border-slate-100">
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          ...
          <button 
            type="submit"
            className="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-2.5 rounded-lg transition-colors mt-2 shadow-md shadow-blue-500/20"
          >
            Iniciar Sesión
          </button>
        </form>
